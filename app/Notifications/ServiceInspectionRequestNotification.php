...
class ServiceInspectionRequestNotification extends Notification
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    protected $data;

    public function __construct(public ServiceInspection $inspection)
    {
        $this->data['title'] = [
            'ar' => 'طلب معاينة جديد',
        ...
        $this->data['body'] = [
            'ar' => 'لقد طلب ' . $inspection->user?->name . ' معاينة لخدمة ' . $inspection->service?->getTranslation('name', 'ar'),
            'en' => $inspection->user?->name . ' requested an inspection for ' . $inspection->service?->getTranslation('name', 'en'),
        ];
